...
class CompletionCmsConfig
{
    private const XML_PATH_CMS_FIELDS = 'riverstone_aicontentgenerator/cms_settings/category_fields';
    private const XML_PATH_CMS_PAGE_ENABLED = 'riverstone_aicontentgenerator/cms_settings/enabled';
    private const META_CMS_PAGE = 'cms_page_form.cms_page_form.search_engine_optimisation';
    ...
    public function getConfig(): array
    {
        ...
        if ((!$this->config->getValue(Config::XML_PATH_ENABLED)) || (!$this->scopeConfig->getValue(self::XML_PATH_CMS_PAGE_ENABLED, ScopeInterface::SCOPE_STORE))) {
            foreach ($getDefaultAttribute as $option) {
                $value = $option['value'];
                $productPagePath = self::META_CMS_PAGE;
                $targets[$value] = [
        ...
        $selectedAttributes = $this->getCmsFields();
        $notPresent=[];
        ...
        foreach ($selectedAttributes as $attribute) {
            ...
            $targets[$attribute['field_name']] = [
                ...
                'component' => 'Riverstone_AiContentGenerator/js/cms',
    ...
    public function getCmsFields()
    {
        ...
        $config = $this->scopeConfig->getValue(self::XML_PATH_CMS_FIELDS, ScopeInterface::SCOPE_STORE);
        ...
        foreach ($unserializedata as $key => $row) {
            $attributeArray[] = ['field_name' => $row['cms_field'][0] , 'prompt' => $row['prompt']];
        }
